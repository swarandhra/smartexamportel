...
import { getSettings, getOfflineQueue, clearOfflineQueue, addToOfflineQueue } from './db.js';

// Send student's result to the configured Google Apps Script Web App URL
export async function saveResultToGoogleSheet(resultData) {
  ...
  // Format data specifically for the sheet columns
  const payload = {
  ...
  try {
    // If browser says we are offline, fail fast to queue it
    if (!navigator.onLine) {
      throw new TypeError('Failed to fetch (offline)');
    }

    // Google Apps Script requires redirection handling.
    // Fetch supports redirects by default, but we send as 'text/plain' or 'no-cors' 
    // to bypass preflight issues if the server script is not structured to handle OPTIONS.
    const response = await fetch(url, {
      method: 'POST',
      mode: 'no-cors', // Bypasses CORS blocks since we only need to write and do not read custom headers
      headers: {
    ...
    // Note: with mode: 'no-cors', the response status is 0, which is normal.
...
async function saveResultToGoogleSheetWithoutQueueing(resultData) {
  const settings = getSettings();
  const url = settings.googleAppsScriptUrl;
